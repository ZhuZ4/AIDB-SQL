...
class ToolCallManager:
    ...
    def check_before_call(
        ...
        # 诊断工具使用更严格的上限，防止纠正循环膨胀
        max_for_this_tool = (
            self.MAX_CALLS_PER_DIAGNOSTIC_TOOL
            if tool_name in self.DIAGNOSTIC_TOOLS
            else self.MAX_CALLS_PER_TOOL
        )
        if tool_count >= max_for_this_tool:
            reason = self._terminate_session(
                ctx,
                f"工具 '{tool_name}' 调用次数已达上限 ({max_for_this_tool} 次)。"
                "请使用已获取的信息，无需重复查询。",
            )
            return False, reason
    ...
    def record_call(
        ...
        # 记录 SQL 查询（无论成功还是失败，都记录以防止相同查询被反复重试）
        if query and tool_name == "sql_db_query":
            normalized_query = self._normalize_query(query)
    ...
    def get_stats(self, session_id: str) -> Dict:
        ...
        return {
            ...
            "recent_tool_sequence": [item[0] for item in ctx.recent_tool_calls],
    ...
    def _detect_loop_pattern(
        ...
        - 终结性工具（save_report）不纳入模式检测序列
        """
        # 终结性/辅助工具不纳入循环模式检测
        # sql_self_correct 是编排器，内部会批量触发子工具调用（sql_schema_validate、
        # sql_join_validate 等），这些子调用不应污染 LLM 层面的模式检测序列
        EXCLUDED_TOOLS = {
            "save_report", "load_skill", "list_skills",
            "sql_self_correct", "sql_schema_validate", "sql_join_validate",
            "sql_clause_validate", "sql_error_classify", "sql_diff_report",
        }
        ...
        recent_tools = [
            ...
            if t[0] not in EXCLUDED_TOOLS
        ]
        if current_tool not in EXCLUDED_TOOLS:
