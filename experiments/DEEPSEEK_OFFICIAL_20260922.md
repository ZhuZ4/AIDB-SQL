...
最新停止状态：第二轮在 **2026-09-22 04:34:31（Asia/Shanghai）** 收到官方 `402 / Insufficient Balance` 后停止，已保存 66 条终态（64 提交、2 语义失败），234 题尚未开始。q1238 已在余额错误前正式提交，因此保留该终态；`progress.json` 的 65 是停止分支未刷新造成的旧值，以 state 和 66 条 predictions 为准。本轮没有完整评分，新系列未登记候选。部分交付及从 q1241 接续的准确命令见 `RESOURCE_STOP_DEEPSEEK_20260922.md`。额度不足期间不派发模型请求，也不安排自动充值或重启。
...
| 基线第 2 轮 | `DSF_20260922_B0_02` | 同一 300 题；余额停止，66 终态 / 234 pending |
